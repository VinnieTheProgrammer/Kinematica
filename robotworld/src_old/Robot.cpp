...
namespace Model
{
	...
	void Robot::drive()
	{
		try
		{
			...
			while (position.x > 0 && position.x < 1000 && position.y > 0 && position.y < 1000 && pathPoint < path.size()) // @suppress("Avoid magic numbers")
			{
				// Do the update
				...
				while(perceptQueue.size() > 0)
				{
					...
					if(percept)
					{
						...
						}else
						{
							...
							for(int pointIndex = 0; pointIndex < distancePercepts->pointCloud.size(); ++pointIndex) {
								currentRadarPointCloud.push_back(distancePercepts->pointCloud[pointIndex]);
							}
						}
					}else
					{
